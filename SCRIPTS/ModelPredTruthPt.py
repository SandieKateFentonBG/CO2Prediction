...
def plotPredTruth(displayParams, modelGridsearch, DBpath, TargetMinMaxVal, fontsize = 14, ):
    ...
    if displayParams['showPlot'] or displayParams['archive']:
        ...
        title = str(modelGridsearch.modelPredictor) + '\n' + '- BEST PARAM (%s) ' % modelGridsearch.Param \
        ...
        if displayParams['archive']:
            path, folder, subFolder = DBpath, "RESULTS/", reference + 'VISU/GS/Pred_Truth'
            import os
...
def predTruthCombined(displayParams, GSs, DBpath, scatter=False, fontsize=14): #todo: unchecked function
    ...
    if displayParams['showPlot'] or displayParams['archive']:
        ...
        for GS in GSs:
            labels.append(GS.modelPredictor)
            yPreds.append(GS.yPred)
            label.append(GS.modelPredictor)
            y.append(GS.yPred)
